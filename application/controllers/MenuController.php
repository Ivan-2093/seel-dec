...
class MenuController extends CI_Controller
{
    ...
    {
        $idSubmenuEditar = $this->input->POST('idMenuDelete');
        if ($idSubmenuEditar != "") {
            $data_where_menu = array(
                'id_submenu' => $idSubmenuEditar
            );
            $data_where_pm = array(
                'submenu_id' => $idSubmenuEditar
            );
            if ($this->MenusModel->getMenuPerfilByIdSubmenu($data_where_pm)->num_rows() == 0) {

                if ($this->MenusModel->deleteSubmenuById($data_where_menu)) {

                    $array_response = array(
                        'response' => 'success',
                        'title' => 'Exito!',
                        'message' => 'Submenu eliminado exitosamente!'
                    );
                } else {
                    $array_response = array(
                        'response' => 'error',
                        'title' => 'Error!',
                        'message' => 'Error al eliminar el Submenu seleccionado!.'
                    );
                }
            } else {
                $array_response = array(
                    'response' => 'warning',
                    'title' => 'Advertencia!',
                    'message' => 'No se puede eliminar el Submenu seleccionado, ya que se encuentra asigando al menos a un perfil.'
                );
            }
        } else {
            $array_response = array(
                'response' => 'warning',
                'title' => 'Advertencia!',
                'message' => 'No se encontro identificacion del Submenu seleccionado'
            );
        }
        echo json_encode($array_response);
    }
